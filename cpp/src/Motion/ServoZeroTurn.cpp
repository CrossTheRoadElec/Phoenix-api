...
ServoZeroTurn::ServoZeroTurn(CTRE::Drive::ISmartDrivetrain *driveTrain, CTRE::Drive::Styles::Smart smartStyle,
...
bool ServoZeroTurn::Set(float targetHeading, float headingTolerance, float maxOutput){
    _maxOutput = maxOutput;
...
bool ServoZeroTurn::ZeroTurn(float targetHeading, float headingTolerance){
    if (servoParams->P == 0 && servoParams->I == 0 && servoParams->D == 0)
    	HAL_SendError(true, 1, false, "CTR: Servo Zero Turn has no PID values, cannot turn", "", "", true);
    ...
    float X = ((headingError) * servoParams->P) - ((headingRate) * servoParams->D);
    X = -X;
    ...
    switch (_selectedStyle)
    {
        case CTRE::Drive::Styles::Smart::PercentOutput:
            _driveTrain->Set(CTRE::Drive::Styles::Smart::PercentOutput, 0, X);
            break;
        case CTRE::Drive::Styles::Smart::Voltage:
            _driveTrain->Set(CTRE::Drive::Styles::Smart::Voltage, 0, X);
            break;
        case CTRE::Drive::Styles::Smart::VelocityClosedLoop:
            _driveTrain->Set(CTRE::Drive::Styles::Smart::VelocityClosedLoop, 0, X);
            break;
    }
