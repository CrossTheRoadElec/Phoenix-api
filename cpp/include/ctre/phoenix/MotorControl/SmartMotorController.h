...
#include "ctre/phoenix/CCI/TalonSRX_CCI.h"
...
class SmartMotorController : public IMotorController,
 ...
 public:
  ...
  explicit SmartMotorController(int deviceNumber);
  explicit SmartMotorController(int deviceNumber, int controlPeriodMs);
  DEFAULT_MOVE_CONSTRUCTOR(SmartMotorController);
  ...
  bool IsModePID(ControlMode::SmartControlMode mode) const;
  ...
  bool GetForwardLimitOK() const;
  bool GetReverseLimitOK() const;
  ...
  void SetControlMode(ControlMode::BasicControlMode mode);
  void SetControlMode(ControlMode::SmartControlMode mode);
  /**
   ...
   */
  void SetTalonControlMode(ControlMode::SmartControlMode controlMode);
  ControlMode::SmartControlMode GetControlMode()  const;
 ...
 */
 //SmartMotorControllerSRX & GetLowLevelObject() { return *m_impl; }
 void* GetLowLevelHandle() {return m_handle;}

 void Follow(IMotorController *masterToFollow){
	SetControlMode(ControlMode::SmartControlMode::kFollower);
	Set(masterToFollow->GetDeviceID());
 }
 ...
 private:
  ...
  ControlMode::SmartControlMode m_controlMode = ControlMode::SmartControlMode::kPercentVbus;
  ControlMode::SmartControlMode m_sendMode = ControlMode::SmartControlMode::kPercentVbus;

  void ApplyUsageStats(UsageFlags Usage);
  UsageFlags ControlModeUsage(ControlMode::SmartControlMode mode);
   ...
   * @see Set()
   */
  void ApplyControlMode(ControlMode::SmartControlMode mode);
  /**
